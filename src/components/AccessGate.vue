...
<template>
  ...
  >
    ...
    <form class="mx-auto w-full max-w-[20rem]" @submit.prevent="onSubmit">
      ...
      <!-- ⛔ Эта строка обязана здесь стоять. Экран входа, который молчит о
           том, что он не защита, обещает больше, чем делает. -->
      <p
        class="kh-balance mx-auto mt-5 max-w-[19rem] text-center text-[0.75rem] leading-relaxed"
        :style="{ color: 'var(--text-muted)' }"
      >
        Пароль здесь — заслон от случайного гостя, а не защита: репозиторий открытый,
        и данные в нём видно без входа. Так решено осознанно.
      </p>
    </form>
